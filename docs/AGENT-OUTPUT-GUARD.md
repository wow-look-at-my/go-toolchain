...
four lines of build metadata -- no coverage report, no test result. `version` is also what this
...
- **Pipe** — allowed only when `isHarnessPipeReader` says the reader is the
  ...
  string alone, which both ends of a pipe share. `fdAccessMode` reads that candidate's O_ACCMODE from
...
- **Socket / anon-inode** — gets the exact same `isHarnessPipeReader` chance a
  pipe gets, but NOT via. `socketPeerPID` uses `getsockopt(SOL_SOCKET, SO_PEERCRED)` on the
  ...
  instead of printed to the terminal" — the pipe allowance existed.
...
- **UNIX-domain socket** — unlike a FIFO, `getsockopt(SOL_LOCAL,
  ...
  `child_process`'s stdio) is typically a socketpair, not a bare FIFO.
...
slave answers yes to it unconditionally. `script -qec "go-toolchain" out.log`
...
-- it was a recording device wearing a terminal's clothes.
...
detection itself uses) and checks each comm against a short. `script`, `scriptreplay`, `ttyrec`, `ttyplay`,
...
reproduces the reported bypass verbatim, for real.

## Build constraints

Released "linux" binaries are GOOS=cosmo fat-APE slot copies, so the
classifier for that release target MUST.
...
APE rather than a native `GOOS=darwin` build. Its build tag is satisfied at BUILD time by the cosmo
target, but at RUNTIME on darwin `/proc` does not exist, the readlink fails. The guard is engaged (the
...
not earned a refusal. Once the
primitives exist, a descriptor they cannot answer for is no longer "no
mechanism" but a *failed probe* — and the honest reply is to refuse. Keeping the two apart in the code is what makes that change
...
1. ~~**`wow-look-at-my/is-this-an-agent`.**~~ **MERGED.** Its `proc.go` was
   ...
   **It moved none of the five failing tests, and could not have.** `inspectFD`
   ...
   — and returns before `agent.CommPPID` is called at all. Necessary, not
...
3. **Here, and only after 2.** `inspectFD` dispatches on `hostos.GOOS()` and
   ...
   arm64-apple variadic stack-passing fix internally.

Do not attempt item 3 against unmerged primitives. The choice there is a
temporary ref pin (forbidden) or code that cannot be run.
...
- `src/cmd/claudeguard_ptywrap_test.go` — `ptyWrapperAncestor`'s walk against
  ...
  matches, a fully-resolvable wrapper-free chain answers not-found.
...
- `src/cmd/claudeguard_darwin_test.go` (`//go:build darwin`) — the darwin
  ...
  `socketPeerPID`'s raw getsockopt call against a real socketpair. Only runs when built and
...
- `.github/dats-fixtures/agent-output-guard.dats`, one file copied by every
  ...
  (smoke-macos) restricts writes to exactly that directory.
- `.github/dats-fixtures/socketharness.go` spawns the binary with its stdout
  ...
  than a format the kernel loads.
