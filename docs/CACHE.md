...
The loose-file `LocalCache` is the default tier, and the packed FUSE tier is
opt-in:

```mermaid
flowchart LR
    A["NewLocalStore(dir)"] --> Z{"GOCACHE_FUSE=1?"}
    Z -- no --> E["LocalCache<br/>(loose files)"]
    Z -- yes --> B{"newFuseCache:<br/>mount succeeds?"}
    B -- yes --> C["FuseCache<br/>(virtual filesystem)"]
    B -- "no: cosmo, Windows, no /dev/fuse,<br/>no permission, no fusermount" --> E
```

- **Why loose is the default.** `fusecache.go` is `(linux && !cosmo) || darwin`
  because go-fuse does not compile for `GOOS=cosmo`, and every shipped binary
  is a cosmo APE. A packed default therefore gave a `go run ./src` build a
  `packs/` store the installed binary could not read — two disjoint formats
  under one `buildcache/`, each flavor starting cold behind the other. The
  default is the tier every binary can open.
- **`GOCACHE_FUSE=1`** opts a build into packs. It pays off only where every
  run uses the same binary flavor; mixing flavors splits the cache again.
- **The chosen tier is always logged**, on both paths. A tier that changes
  under a build without a word is how the split above went unnoticed for a
  whole release.
...
├── <aa>/                    # default tier: 256 shard dirs
│   ├── v1<actionID>         # body
│   └── v1<actionID>.meta    # sidecar
├── mnt/                     # GOCACHE_FUSE=1 only: mount point (empty when unmounted)
│   └── <outputID>           # virtual files; reads served from packs
└── packs/                   # GOCACHE_FUSE=1 only
    ├── pack-000001.data     # append-only records (see format above)
    └── pack-000002.data     # ... after rotation at 1 GiB
```

The two are disjoint: nothing reads a body written by the other tier. That is
why the default has to be the tier every binary can open — see **Fallback and
portability** above.
...
`cacheprog`, either tier). `mnt/` (a possibly-mounted mountpoint), the
