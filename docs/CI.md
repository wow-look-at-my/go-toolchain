...
Every assertion this job makes lives in .github/dats-fixtures/smoke-windows.dats. The job checks the repo out for that file, downloads the build hand-off, and runs the suite. A workflow step schedules work. The harness holds the assertions. So an engineer can run them without pushing a commit.
...
The same whole-pipeline assertion smoke-linux and smoke-macos make: the shipped APE tidies, vets, tests. The shipped APE builds a synthetic consumer module, and prints "Build successful". The module is `inputs.files` entries. So the fixture carries it instead of a heredoc in a shell step.
