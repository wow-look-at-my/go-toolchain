...
`go.mod`'s `go 1.27` is the fix for the second one. And it is a floor rather than a preference. The fork is the bootstrap compiler in CI, so its own `go/types` is what gets linked into the binary that does the type-checking. The directive keeps a stock toolchain from claiming it can build this. Built against go1.26 it fails both ways. The import panics as above, and reading the same package's source instead only trades the panic for `method must have no type parameters` plus. Keep this directive at or above the fork's Go version.
