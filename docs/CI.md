...
`host-build` bootstraps from the previous release: the last go-toolchain a green master run published, downloaded from buildhost the way every consumer downloads it, builds the pipeline once, and the pipeline then builds itself in passes (`selfbuild.go`). Each pass compiles the fork's standard library from the submodule with `go tool embedstd`, links the tree, and appends the blob with `GOCOSMOAPPEND`. The last two passes must be byte-identical. A release that links no go command cannot compile the tree, so the job then runs the from-scratch steps in place: a stock Go runs the submodule's `make.bash` and that go command builds the bootstrap pipeline with the flags the pipeline builds itself with, `-trimpath` and `CGO_ENABLED=0`, so its compiler reports the tool ID the passes do and the passes read its cache entries.

`bootstrap-scratch` runs the from-scratch path on every push regardless, off the critical path, and hands its APE off as `ape-scratch`. `identical` holds it to `build`'s bytes. Two bootstraps with no compiler in common must reach the same fixed point, which is a stronger statement about the build than either alone makes, and a release that turns out unable to build the tree leaves a bootstrap that can.

A linked tool's ID names its own code, not the binary carrying it (the fork's `linkedToolIDs`), so a rebuild of the pipeline that touches none of the fork's packages keeps every cache entry the previous binary wrote, and the org's shared cache serves one release's compile to the next.
...
`src/test/test.go` bounds every test binary at five minutes. The number is a property of one package. `src/cmd` holds around tests that call `t.Chdir` or `t.Setenv`. Under the fork either call runs the rest of its test in a child process.
...
Every assertion this job makes lives in .github/dats-fixtures/smoke-macos.dats: host detection, the whole pipeline over a synthetic consumer. The two unsandboxed socket cases. A workflow step schedules work. The harness holds the assertions.
