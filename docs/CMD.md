...
and a knob that turned it off would eventually be set and left set.
...
smoke jobs. That one carve-out is pinned to an exact repository name
precisely so it cannot become a general opt-out.
...
bootstrap-failure exit, and before the "Up to date, nothing to do" fast-exit. The check never blocks
...
`GO_TOOLCHAIN_GITHUB_API_URL` overrides that base.
...
the run's flags, the run's environment.
...
- **The environment.** An env-gated test or benchmark switched on between two
  ...
  folded in except `volatileEnv` — `_`, `OLDPWD`. The
...
agent AND its stdout is hidden.
...
the subprocess's stdout is the GOCACHEPROG protocol channel cmd/go parses.
...
- **The output watchdog** is mirrored via stdlib `syscall` (`watchdog_cosmo.go`;
  ...
  supported off-switch that keeps the build on its real stdio.
- **The GOCACHEPROG self-exec** goes through `cacheProgCommand`
  ...
  header + compiled loader), keeps its MZ magic. Every other platform keeps
...
- **The persistent outdated-deps cache** is behind the `depsCache` interface,
  ...
  engine, and it is compiled into every binary. `close` merges onto the file
...
set. Default `linux/amd64,darwin/arm64,windows/amd64`; `all` leaves the
...
with a sentinel value, which only an aware toolchain echoes back. The artifact is still correct there: a superset APE runs on
every platform claimed, and for the default set it is not even larger.
...
next to the APE, naming the file, its platform set.
...
build when one exists, else the APE. Without
...
accumulated in CLAUDE.md. Unlike `docs/ACTION.md` and `docs/CI.md`, these two did. Both are above.*
