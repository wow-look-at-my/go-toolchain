...
Things end a run: any other statement, a write to a different writer, and a write whose text is computed. `b.WriteByte(c)` inside an escaper writes one byte of a value, and no template renders it.
