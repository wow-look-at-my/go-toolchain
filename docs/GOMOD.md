...
parallel unless one takes the serial barrier. A test that changed the process directory moved it under every test beside it, which showed up.
...
header, so a k8s-style copyright block hid the package clause and the main package was silently not built).
