...
**`apt-depends` is its own input, not an `autorelease_args` key.** A Debian relation holds spaces and commas, which the `autorelease_args` parser splits on. The value goes to buildhost-publish's `apt_depends` unchanged. buildhost validates it. And an invalid value fails the publish.
