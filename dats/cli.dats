...
tests:
	...
	#
	# A host with a classifier must REFUSE. The banner that says the guard is
	# blind used to be accepted here as an equal answer, which made this check
	# unable to tell a working guard from one that sees nothing: dats prints a
	# passing test's output nowhere, so every darwin run since has been green
	# without anyone learning which of the two it got.
	#
	# The sandbox is not the reason to expect a blind darwin run. dats' seatbelt
	# profile is `(allow default)` with `file-write*` and, when a suite asks for
	# it, `network*` denied. Naming a pipe's reader needs neither.
