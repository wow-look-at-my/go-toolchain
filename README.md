...
- **Dependency graph submission** — automatically submits a dependency snapshot to GitHub's Dependency Submission API in CI when a GitHub token (`GITHUB_TOKEN`/`GH_TOKEN`, needs `contents: write`) is available, populating the repository's dependency graph for vulnerability alerts and Dependabot. Skipped silently when no token is present, so a build that doesn't plumb a token stays quiet
...
17. Submits a dependency snapshot to GitHub's Dependency Submission API (when running in CI with a token available — `$CI`, `$GITHUB_REPOSITORY`, and `$GITHUB_SHA` are set and `$GITHUB_TOKEN`/`$GH_TOKEN` is non-empty), populating the repository's dependency graph with all direct and indirect Go module dependencies for vulnerability scanning and Dependabot alerts. With no token the submission is skipped silently (it isn't configured for that run), not reported as a failure
