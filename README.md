...
| `autorelease_args`  | string   | `''`       | Extra publish options forwarded to the buildhost publish, as whitespace/comma-separated `key=value` pairs. Recognized: `brew_service=true\|false` (declare Homebrew `service do` support on the published project). Unknown keys fail the build; empty forwards nothing |
