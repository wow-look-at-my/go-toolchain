...
- **Dependency checking** — detects outdated dependencies and auto-updates same-org deps; a dependency can instead be pinned to follow a specific branch with a `// go-toolchain:branch=<name>` comment (the "`//branch` comment"). See [docs/DEPS.md](docs/DEPS.md)
