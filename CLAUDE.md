...
- `src/vet/` — custom vet checks (assert normalization, unused imports, gotest.tools migration, testify import rewrite fork→upstream with vendor resync, and the `testifycast` analyzer that inserts explicit type conversions into cross-type `assert`/`require` `Equal`/`NotEqual` operands so they pass against upstream testify). **Fix mode vs check mode**: `root.go` sets `fix := os.Getenv("CI") == ""`, so locally the fixers rewrite the tree in place, while on CI they run **read-only** — `RunGofmt`, `FixTestifyImports`, `MigrateGotestTools` (and the `testifycast` edits, surfaced as diagnostics) all return a hard error listing the offending files instead of writing. This keeps CI from passing green on a tree the local autofixer would have changed (e.g. a lingering `wow-look-at-my/testify` fork import). Any new in-place fixer MUST take a `fix bool` and error in check mode the same way, or CI will silently stop enforcing it.
