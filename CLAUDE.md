...
- `src/cmd/` — CLI commands (root, matrix, bench, lint, install, version, release, ignore/unignore, cacheprog) using Cobra; also includes dependabot (automatic dependency graph submission to GitHub in CI). The binary does not self-update: it is installed/updated from buildhost (the GitHub Action downloads it with `curl`; end users use a package manager such as Homebrew/npm/APT). `claudeguard.go` (+ `claudeguard_linux.go` / `claudeguard_other.go`) implements the **Claude pipe-filter guard**: the root `PersistentPreRunE` calls `guardAgainstClaudePipeFilter()` (after the `skipCache` early-return, so `cacheprog`/`version`/`install`/`release` are exempt) and aborts with exit 1 when go-toolchain runs under Claude (`CLAUDECODE` env or an ancestor process named `claude`) **and** stdout is piped into a recognized filter (`head`/`tail`/`grep`/`sed`/`awk`/…). The filter is identified on Linux by matching the stdout pipe's inode against other processes' `/proc/<pid>/fd` entries; non-Linux is a no-op. Bypass with `GO_TOOLCHAIN_ALLOW_PIPE_FILTER=1`
