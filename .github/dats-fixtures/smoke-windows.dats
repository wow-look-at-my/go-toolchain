# The windows smoke suite. These assertions used to be bash inside the
# smoke-windows job (.github/workflows/ci.yml); a workflow step schedules work,
# it is not a test harness, so they live here where an engineer can run them.
#
# The job runs this file with --no-sandbox. NT has neither bwrap nor seatbelt,
# and windows-latest's docker daemon serves windows containers, so no backend
# here can build a sandbox. Opting out is the decision of whoever starts the
# run, which is why this file never tries to make it.
#
# ../../dist/go-toolchain is the published fat APE the build job handed off.
# The .exe destination is load-bearing: NT starts a program by that suffix.
