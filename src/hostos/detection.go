...
// hostSignalFunc is the seam for the gosmopolitan __hostos signal, wired in hostos_cosmo.go (only a cosmo build defines it).
...
// guessedHostBanner is the warning, held as a single document. Its only value
// is the OS the fallback assumed.
...
type Detection struct {
	...
	OS string
	// Method names the evidence: compiled, runtime, uname, coreservices, procfs, or default (a guess).
	Method string
	// Uname is the sysname the uname syscall reported, empty when it did not answer.
	Uname string
}
