...
func parseProxyConfig() *proxyConfig {
	...
	if err != nil {
		logx.Logf("proxy: GO_PROXY_CONFIG decode error: %v", err)
		return nil
	}
	var cfg proxyConfig
	if err := json.Unmarshal(data, &cfg); err != nil {
		logx.Logf("proxy: GO_PROXY_CONFIG parse error: %v", err)
		return nil
	}
	return &cfg
}
...
func writeNetrc(host, user, password string) {
	...
	if err != nil {
		logx.Logf("proxy: netrc tmp: %v", err)
		return
	}
	tmpPath := tmp.Name()
	if _, err := tmp.WriteString(newContent); err != nil {
		tmp.Close()
		os.Remove(tmpPath)
		logx.Logf("proxy: netrc write: %v", err)
		return
	}
	if err := tmp.Chmod(0600); err != nil {
		tmp.Close()
		os.Remove(tmpPath)
		logx.Logf("proxy: netrc chmod: %v", err)
		return
	}
	if err := tmp.Close(); err != nil {
		os.Remove(tmpPath)
		logx.Logf("proxy: netrc close: %v", err)
		return
	}
	if err := os.Rename(tmpPath, netrcPath); err != nil {
		os.Remove(tmpPath)
		logx.Logf("proxy: netrc rename: %v", err)
	}
}
