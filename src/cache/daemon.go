	...
	if d.remote != nil {
		...
		if wb, ok := d.remote.(*WebBackend); ok {
			...
			if hits > 0 || puts > 0 || missTotal > 0 {
				fmt.Fprintf(os.Stderr, "cacheprog: web summary: hits=%d puts=%d misses=%d (not-in-index=%d http-404=%d http-err=%d no-outputid=%d read-body=%d decompress=%d checksum=%d buildid=%d modindex=%d network=%d circuit-open=%d skipped-empty-index=%d skipped-not-in-index=%d skipped-batch-backoff=%d reclaimed-404=%d) put-skipped: known=%d modindex=%d buildid=%d\n",
					hits, puts, missTotal, notInIndex, http404, httpErr, noOutputID, readBody, decompress, checksum, buildID, moduleIndex, network, circuitOpen, skippedEmptyIndex, skippedNotInIndex, skippedBatchBackoff, reclaimed404, putSkippedKnown, putRefusedModIndex, putRefusedBuildID)
			}
		}
		d.remote.Close()
