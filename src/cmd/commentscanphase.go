...
// commentScan is the comment repair, running beside the phases that do not
// read what it rewrites. Every decision about comments is slopfix's.
...
// startCommentScan sweeps root on its own goroutine and answers a handle.
//
// The caller must have found a go.mod already: a repair is a write, and a run
// begun where there is no module has no business rewriting the prose it finds.
//
// slopfix renames each repaired file into place, so this is safe beside go mod
// tidy and go generate. It is NOT safe beside vet, which rewrites the same
// files, and the caller joins ahead of it.
