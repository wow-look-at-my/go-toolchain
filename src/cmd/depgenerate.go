...
// A directive whose generator the module never shipped is dropped: it cannot
// run here or anywhere, so demanding an approval for it asks the consumer to
// authorize a command that then fails. The drop is loud, because the package it
// belongs to builds from what the zip carried and may panic naming the step.
