...
`
	mock.SetResponse("go", []string{"test", "-json", "-timeout=" + testTimeout.String(), "-coverprofile=" + coverFile, "-coverpkg=./...", "-count=1", "./..."}, []byte(testOutput), nil)
...
func TestRunTestsFailure(t *testing.T) {
	...
	mock := runner.NewMock()
	mock.SetResponse("go", []string{"test", "-json", "-timeout=" + testTimeout.String(), "-coverprofile=" + coverFile, "-coverpkg=./...", "-count=1", "./..."}, nil, fmt.Errorf("test failed"))
...
`
	mock.SetResponse("go", []string{"test", "-json", "-timeout=" + testTimeout.String(), "-coverprofile=" + coverFile, "-coverpkg=./...", "-count=1", "example.com/proj/pkg1"}, []byte(testOutput), nil)
